...
class CheckpointMonitor:
    ...
    def mark_in_progress(self, checkpoint_dir: Path, seed: Optional[int] = None):
        """Mark checkpoint (or checkpoint+seed) as being validated (in progress)."""
        if seed is None:
            ...
            if "by_seed" not in entry:
                entry["by_seed"] = {}
            entry["by_seed"][str(seed)] = {
                "status": "in_progress",
                "started_at": datetime.now().isoformat(),
                "success": None,
                "metrics": None,
    ...
    def mark_validated(
        ...
        """Mark checkpoint (or checkpoint+seed) as validated (completed)."""
        entry = self.validated_checkpoints.get(checkpoint_dir.name, {})
        if seed is None:
            entry.update({
                "status": "completed",
    ...
    def is_seed_validated(self, checkpoint_dir: Path, seed: Optional[int]) -> bool:
        """Return True if this (checkpoint, seed) is already validated. When seed is None, check legacy entry status."""
        entry = self.validated_checkpoints.get(checkpoint_dir.name, {})
        if seed is None:
            return entry.get("status") == "completed"
        return entry.get("by_seed", {}).get(str(seed), {}).get("status") == "completed"

    def all_seeds_validated(self, checkpoint_dir: Path, test_seeds: List[Optional[int]]) -> bool:
        """Return True if all seeds in test_seeds are validated for this checkpoint."""
        for s in test_seeds:
            if not self.is_seed_validated(checkpoint_dir, s):
                return False
        return True
...
class ValidationRunner:
    ...
    def _generate_image_batch(self, batch_images, all_prompts_data, unet, text_encoder, vae, global_step, batch_start_idx):
        ...
        # Generate latents with unique seeds (optional validation_seed_offset for multi-seed test runs)
        seed_offset = self.args.get("validation_seed_offset", 0)
        generators = [
            torch.Generator(device=self.accelerator.device).manual_seed(
                global_step * 10000 + self.accelerator.process_index * 1000 + batch_start_idx + i + seed_offset
            )
    ...
    def _add_hcn_conditioning(self, text_embeddings, batch_data_list):
        ...
        for (batch_data, batch_idx) in batch_data_list:
            ...
            if has_demographics and has_age:
                ...
                if use_continuous_age and "age_continuous" in batch_data:
        ...
        if sex_indices and race_indices:
            ...
            if use_continuous_age and age_continuous_values:
    ...
    def _load_validation_images_from_disk(self, global_step):
        ...
        # Use test_images directory in test mode (or test_images_seeds for multi-seed), train_images in train mode, etc.
        output_dir = Path(self.args["output_dir"])
        if self.args.get("test_mode", False):
            images_dir_name = self.args.get("test_images_dir_name", "test_images")
        ...
        if self.args.get("validation_seed_offset") is not None:
            validation_images_dir = validation_images_dir / f"seed_{self.args['validation_seed_offset']}"
    ...
    def _save_validation_images(self, all_synthetic_images, all_real_images, all_labels, global_step):
        ...
        try:
            ...
            # Use test_images (or test_images_seeds for multi-seed) in test mode, train_images in train mode, etc.
            output_dir = Path(self.args["output_dir"])
            if self.args.get("test_mode", False):
                images_dir_name = self.args.get("test_images_dir_name", "test_images")
            elif self.args.get("train_mode", False):
...
def main():
    ...
    parser.add_argument("--test_seeds", type=str, default=None, help="Comma-separated list of generation seeds for test mode (e.g., '42,123,456'). Each seed runs a separate validation with different random generations. If not set, a single run is performed (current behavior).")
    ...
    if test_mode:
        ...
        # Parse test_seeds for multi-seed test runs (each seed = different generation)
        test_seeds = [None]  # default: single run, backward compatible
        if args.test_seeds:
            try:
            ...
            print(f"Test mode: Will run {len(test_seeds)} seed(s) per checkpoint: {test_seeds}")

        # Multi-seed: use separate manifest, images dir, and sync dir so we don't touch old test_manifest.json / test_images
        use_multiseed_paths = len(test_seeds) > 1 or (len(test_seeds) == 1 and test_seeds[0] is not None)
        if use_multiseed_paths:
            config["test_images_dir_name"] = "test_images_seeds"
            config["test_sync_dir_name"] = "test_sync_seeds"
            if args.manifest_file == "validation_manifest.json" or args.manifest_file == "test_manifest.json":
                args.manifest_file = "test_manifest_seeds.json"
            print(f"Test mode (multi-seed): Using separate manifest and dirs (test_manifest_seeds.json, test_images_seeds/, test_sync_seeds/)")
        else:
            # Single-run test: default test_manifest.json and test_images
            if args.manifest_file == "validation_manifest.json":
    ...
    try:
        ...
        if load_images_from_dir and not test_mode and not train_mode and not gt_mode:
            ...
            if config.get("test_mode", False):
                images_dir_name = config.get("test_images_dir_name", "test_images")
        ...
        # Use test_sync (or test_sync_seeds for multi-seed) in test mode, train_sync in train mode, etc.
        if test_mode:
            sync_dir_name = config.get("test_sync_dir_name", "test_sync")
        elif train_mode:
        ...
        if test_mode:
            ...
            logger.info(f"Seeds per checkpoint: {test_seeds}")
            if test_continuous:
                logger.info(f"Check interval: {args.check_interval}s")
            logger.info(f"Manifest file: {args.manifest_file}")
            ...
            # Helper function to process a single checkpoint (runs all requested seeds; loads checkpoint once)
            def process_checkpoint(checkpoint_step, checkpoint_dir):
                """Process a single checkpoint - runs validation for each seed in test_seeds. Returns True if all seeds succeeded."""
                logger.info(f"\n{'='*60}")
                logger.info(f"TEST MODE: Validating checkpoint: {checkpoint_dir.name} (step {checkpoint_step}), seeds: {test_seeds}")
                logger.info(f"{'='*60}")
                ...
                if step is None or checkpoint_dir_str is None:
                    ...
                    if accelerator.is_main_process:
                        for seed in test_seeds:
                            monitor.mark_validated(checkpoint_dir, {}, success=False, num_images=0, seed=seed)
                    return False
                
                checkpoint_dir = Path(checkpoint_dir_str)
                all_ok = True

                # Load checkpoint once (shared across all seeds)
                try:
                    if not load_images_from_dir:
                ...
                for seed in test_seeds:
                    # Decide skip on main, then broadcast so ALL ranks skip or run together (avoids GPU deadlock)
                    if accelerator.is_main_process:
                        skip_seed = monitor.is_seed_validated(checkpoint_dir, seed)
                    else:
                        skip_seed = False
                    if accelerator.num_processes > 1:
                        skip_tensor = torch.tensor([1 if skip_seed else 0], device=accelerator.device, dtype=torch.long)
                        torch.distributed.broadcast(skip_tensor, src=0)
                        skip_seed = skip_tensor.item() == 1
                    if skip_seed:
                        if accelerator.is_main_process:
                            logger.info(f"Seed {seed} already validated for {checkpoint_dir.name}, skipping")
                        continue

                    seed_label = f" (seed {seed})" if seed is not None else ""
                    logger.info(f"TEST MODE: Running validation for {checkpoint_dir.name}{seed_label}")
                    if accelerator.is_main_process:
                        monitor.mark_in_progress(checkpoint_dir, seed=seed)

                    # Set generation seed for this run (runner.args is config)
                    if seed is not None:
                        config["validation_seed_offset"] = seed
                    else:
                        config.pop("validation_seed_offset", None)
                    ...
                    try:
                        ...
                        if metrics and accelerator.is_main_process:
                            try:
                                ...
                                logger.info(f"✓ Metrics logged at step {checkpoint_step}{seed_label}")
                            except Exception as e:
                                logger.warning(f"Failed to log metrics: {e}")
                                import traceback
                                logger.warning(traceback.format_exc())
                        ...
                        if accelerator.is_main_process:
                            num_images = metrics.get("_num_images", 0) if metrics else 0
                            metrics_for_manifest = {k: v for k, v in metrics.items() if k != "_num_images"} if metrics else {}
                            monitor.mark_validated(checkpoint_dir, metrics_for_manifest, success=True, num_images=num_images, seed=seed)
                        logger.info(f"✓ TEST MODE: Checkpoint {checkpoint_dir.name} seed {seed} validated successfully")
                    except Exception as e:
                        logger.error(f"Failed to validate checkpoint {checkpoint_dir.name} seed {seed}: {e}")
                        import traceback
                        logger.error(traceback.format_exc())
                        if accelerator.is_main_process:
                            monitor.mark_validated(checkpoint_dir, {}, success=False, num_images=0, seed=seed)
                    ...
                    if not checkpoint_dir.exists():
                        ...
                        if accelerator.is_main_process:
                            for seed in test_seeds:
                                monitor.mark_validated(checkpoint_dir, {}, success=False, num_images=0, seed=seed)
                        continue
                    # Broadcast skip so all ranks skip or all run (avoids deadlock)
                    skip_ckpt = False
                    if accelerator.is_main_process:
                        skip_ckpt = monitor.all_seeds_validated(checkpoint_dir, test_seeds)
                    if accelerator.num_processes > 1:
                        skip_t = torch.tensor([1 if skip_ckpt else 0], device=accelerator.device, dtype=torch.long)
                        torch.distributed.broadcast(skip_t, src=0)
                        skip_ckpt = skip_t.item() == 1
                    if skip_ckpt:
                        if accelerator.is_main_process:
                            logger.info(f"Checkpoint {checkpoint_dir.name} already validated for all seeds, skipping")
                        continue
            ...
            # First, process any existing checkpoints that match (including any missing seeds)
            ...
            for checkpoint_step in sorted(checkpoint_numbers):
                ...
                if checkpoint_dir.exists():
                    ...
                    if skip_ckpt:
                        ...
                        if accelerator.is_main_process:
                            logger.info(f"Found existing checkpoint: {checkpoint_dir.name} (will process missing seeds if any)")
                        process_checkpoint(checkpoint_step, checkpoint_dir)
