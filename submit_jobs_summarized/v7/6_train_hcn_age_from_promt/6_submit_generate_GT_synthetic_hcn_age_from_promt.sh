#!/bin/bash
#SBATCH --job-name=generate_GT_synthetic
#SBATCH --output=logs_summarized/logs_v7/6_train_hcn_age_from_promt/generate_GT_synthetic_%j.out
#SBATCH --error=logs_summarized/logs_v7/6_train_hcn_age_from_promt/generate_GT_synthetic_%j.err
#SBATCH --time=7-00:00:00          # 7 days (generation can take a while for large datasets)
#SBATCH --nodes=1                  # Single node
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8         # CPUs per task
#SBATCH --gres=gpu:a5000:7        # GPUs for distributed generation
#SBATCH --mem=230gb                # Memory for generation (7 processes × models)
...
#
# Arguments:
#   checkpoint_path (optional): Path to checkpoint directory (default: outputs_summarized/v7/6_train_hcn_age_from_promt/checkpoint-20000)
#   output_dir (optional): Output directory for synthetic images (default: synthetic_datasets/v7/6_train_hcn_age_from_promt/GT-20000)
...
#   sbatch SCRIPT_NAME checkpoint-30000 ./my_GT_synthetic_output
#
#   # Specify all parameters
#   sbatch SCRIPT_NAME checkpoint-30000 ./my_GT_synthetic_output 10000
...
# Config file path (GT generation config)
CONFIG_FILE="configs_summarized/v7/6_generate_GT_hcn_age_from_promt.yaml"

# Check if config file exists
if [ ! -f "$CONFIG_FILE" ]; then
    echo "ERROR: Config file not found: $CONFIG_FILE"
    echo "Available config files:"
    ls -1 configs_summarized/v7/*.yaml 2>/dev/null || echo "  No config files found in configs_summarized/v7/"
    exit 1
fi

# Default checkpoint (from script creation or use latest)
DEFAULT_CHECKPOINT_NAME="checkpoint-20000"
DEFAULT_CHECKPOINT_PATH="outputs_summarized/v7/6_train_hcn_age_from_promt/checkpoint-20000"
DEFAULT_OUTPUT_DIR="synthetic_datasets/v7/6_train_hcn_age_from_promt/GT-20000"
...
if [ -n "$CHECKPOINT_ARG" ]; then
    ...
    if [[ "$CHECKPOINT_ARG" =~ ^/ ]]; then
        ...
        CHECKPOINT_PATH="outputs_summarized/v7/6_train_hcn_age_from_promt/$CHECKPOINT_NAME"
        # Update default output dir based on checkpoint
        DEFAULT_OUTPUT_DIR="synthetic_datasets/v7/6_train_hcn_age_from_promt/GT-${CHECKPOINT_ARG}"
    fi
else
...
# Launch GT synthetic dataset generation
echo "=========================================="
echo "Starting GT synthetic dataset generation..."
echo "Config file: $CONFIG_FILE"
echo "Checkpoint path: $CHECKPOINT_PATH"
echo "Output directory: $OUTPUT_DIR"
echo "GT dataset: real_data_summarized_four_splits/GT_data"
...
# The config file already has wds_dataset_path set to GT_data
...
# Using 7 GPUs for faster generation (distributes image generation across GPUs)
...
echo "=========================================="
echo "GT synthetic dataset generation finished"
