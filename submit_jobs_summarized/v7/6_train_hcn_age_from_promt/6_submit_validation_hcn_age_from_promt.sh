...
#SBATCH --output=logs_summarized/logs_v7/6_train_hcn_age_from_promt/validation_%j.out
#SBATCH --error=logs_summarized/logs_v7/6_train_hcn_age_from_promt/validation_%j.err
#SBATCH --time=7-00:00:00          # 2 days (validation can run longer)
#SBATCH --nodes=1                  # Single node
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8         # CPUs per task
#SBATCH --gres=gpu:a5000:7               # GPUs for distributed validation
...
CONFIG_FILE="${1:-configs_summarized/v7/6_train_hcn_age_from_promt.yaml}"  # Use first argument or default
