...
#SBATCH --output=logs_summarized/logs_v0/0_train_baseline/generate_GT_synthetic_%j.out
#SBATCH --error=logs_summarized/logs_v0/0_train_baseline/generate_GT_synthetic_%j.err
#SBATCH --time=7-00:00:00          # 7 days (generation can take a while for large datasets)
#SBATCH --nodes=1                  # Single node
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8         # CPUs per task
#SBATCH --gres=gpu:a100:4        # 1 GPU for generation
#SBATCH --mem=150gb                # Memory for generation
...
#
# Arguments:
#   checkpoint_path (optional): Path to checkpoint directory (default: outputs_summarized/v0/0_train_baseline/checkpoint-10000)
#   output_dir (optional): Output directory for synthetic images (default: synthetic_datasets/v0/0_train_baseline/GT-10000)
...
CONFIG_FILE="configs_summarized/v0/0_generate_GT_baseline.yaml"

# Check if config file exists
if [ ! -f "$CONFIG_FILE" ]; then
    echo "ERROR: Config file not found: $CONFIG_FILE"
    echo "Available config files:"
    ls -1 configs_summarized/v0/*.yaml 2>/dev/null || echo "  No config files found in configs_summarized/v0/"
    exit 1
fi
...
DEFAULT_OUTPUT_DIR="synthetic_datasets/v0/0_train_baseline/GT-10000"
...
if [ -n "$CHECKPOINT_ARG" ]; then
    ...
    if [[ "$CHECKPOINT_ARG" =~ ^/ ]]; then
        ...
        DEFAULT_OUTPUT_DIR="synthetic_datasets/v0/0_train_baseline/GT-${CHECKPOINT_ARG}"
    fi
else
