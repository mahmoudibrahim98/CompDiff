...
#SBATCH --output=logs_summarized/logs_fairdiffusion/0_train_baseline_fairdiffusion/generate_synthetic_%j.out
#SBATCH --error=logs_summarized/logs_fairdiffusion/0_train_baseline_fairdiffusion/generate_synthetic_%j.err
#SBATCH --time=7-00:00:00          # 7 days (generation can take a while for large datasets)
#SBATCH --nodes=1                  # Single node
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8         # CPUs per task
#SBATCH --gres=gpu:a100:6               # GPUs for distributed generation
...
DEFAULT_CHECKPOINT_PATH="outputs_summarized/fairdiffusion/0_train_baseline_fairdiffusion/checkpoint-7500"
DEFAULT_OUTPUT_DIR="synthetic_datasets/fairdiffusion/0_train_baseline_fairdiffusion/train-7500-copy1"
...
if [ -n "$CHECKPOINT_ARG" ]; then
    ...
    if [[ "$CHECKPOINT_ARG" =~ ^/ ]]; then
        ...
        CHECKPOINT_PATH="outputs_summarized/fairdiffusion/0_train_baseline_fairdiffusion/$CHECKPOINT_NAME"
    fi
else
