...
#SBATCH --output=logs_summarized/logs_fairdiffusion/0_train_baseline_fairdiffusion/test_%j.out
#SBATCH --error=logs_summarized/logs_fairdiffusion/0_train_baseline_fairdiffusion/test_%j.err
...
#
# Arguments:
#   checkpoints (optional): Comma-separated list of checkpoint numbers (default: 7500,11000)
#                           Example: 30000,50000,70000
#   continuous (optional): Set to "1" or "true" for continuous monitoring, "0" or "false" for one-time processing (default: 1)
...
if [ "$CONTINUOUS_FLAG" = "1" ] || [ "$CONTINUOUS_FLAG" = "true" ] || [ "$CONTINUOUS_FLAG" = "1" ]; then
