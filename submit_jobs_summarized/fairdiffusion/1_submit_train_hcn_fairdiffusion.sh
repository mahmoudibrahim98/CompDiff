...
#SBATCH --output=logs_summarized/logs_fairdiffusion/1_train_hcn_fairdiffusion/training_%j.out
#SBATCH --error=logs_summarized/logs_fairdiffusion/1_train_hcn_fairdiffusion/training_%j.err
#SBATCH --time=7-00:00:00          # 7 days (adjust as needed)
#SBATCH --nodes=1                  # Single node
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=4         # CPUs per task (adjust based on your needs)
#SBATCH --gres=gpu:4               # GPUs (matching your training setup)
...
mkdir -p logs_summarized/logs_fairdiffusion/1_train_hcn_fairdiffusion

# Config file path (adjust as needed)
CONFIG_FILE="configs_summarized/fairdiffusion/1_train_hcn_fairdiffusion.yaml"
